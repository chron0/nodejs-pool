...
function getSQLConfig(callback) {
    let config_data = {};
    if (global.hasOwnProperty('mysql')) {
        global.mysql.query("SELECT * FROM config").then(function(rows){
            rows.forEach(function (row) {
                if (!config_data.hasOwnProperty(row.module)) {
                    config_data[row.module] = {};
                }
                switch (row.item_type) {
                    case 'int':
                        config_data[row.module][row.item] = parseInt(row.item_value);
                        break;
                    case 'bool':
                        config_data[row.module][row.item] = (row.item_value === "true");
                        break;
                    case 'string':
                        config_data[row.module][row.item] = row.item_value;
                        break;
                    case 'float':
                        config_data[row.module][row.item] = parseFloat(row.item_value);
                        break;
                }
            });
        }.then(callback(config_data)));
    }
}

function getConfig(configHost, callback) {
    async.doUntil(
        function (callback) {
            request.post({
                url: configHost,
                form: {key: global.config.configKey}
