...
function monitorNodes() {
    global.mysql.query("SELECT * FROM pools WHERE last_checkin > date_sub(now(), interval 30 minute)").then(function (rows) {
        global.coinFuncs.getLastBlockHeader(function (block) {
            rows.forEach(function (row) {
                    ...
                    } else if (row.blockID > block.height + 3){
                        global.support.sendEmail(global.config.general.adminEmail, "Master server behind in blocks", "The master LMDB/API server is "+row.blockID - block.height+ " blocks behind");
                    }
                }
            );
        });
